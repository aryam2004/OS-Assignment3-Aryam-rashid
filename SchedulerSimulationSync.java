...
// Added imports for synchronization mechanisms
...
// A SHARED RESOURCES - These need synchronization! A
class SharedResources {
    ...
    // Task 1: ReentrantLock for protecting counter variables
    public static final ReentrantLock counterLock = new ReentrantLock();

    // Task 2: ReentrantLock for protecting executionLog ArrayList
    public static final ReentrantLock logLock = new ReentrantLock();

    // Task 3: Binary Semaphore for CPU access control (1 permit)
    public static final Semaphore cpuSemaphore = new Semaphore(1);

    // Method to increment context switch counter
    public static void incrementContextSwitch() {
        // Task 1: Protect critical section with counterLock
        counterLock.lock();
        try {
            contextSwitchCount++;
        } finally {
    ...
    public static void logExecution(String message) {
        // Task 2: Protect critical section with logLock
        logLock.lock();
        try {
...
class Process implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            System.out.println(Colors.RED + " X " + name + " was interrupted while waiting for CPU." + Colors.RESET);
            return;
        }
        ...
        try {
            ...
            try {
                ...
                System.out.println(Colors.RED + "\n X " + name + " was interrupted." + Colors.RESET);
            }
            ...
            System.out.println(Colors.YELLOW + "Ⅱ" + Colors.CYAN + name + Colors.RESET +
            ...
            if (remainingTime > 0) {
                ...
                System.out.println(Colors.BRIGHT_GREEN + "✓" + Colors.BOLD + Colors.CYAN + name + Colors.RESET
                        + Colors.BRIGHT_GREEN + " finished execution!" + Colors.RESET);
            }
    ...
    public void runToCompletion() {
        // Task 3: Acquire semaphore for runToCompletion
        try {
            SharedResources.cpuSemaphore.acquire();
        } catch (InterruptedException e) {
            System.out.println(Colors.RED + " X " + name + " was interrupted." + Colors.RESET);
            return;
        }

        try {
            System.out.println(Colors.BRIGHT_CYAN + "⚡" + Colors.BOLD + Colors.CYAN + name + Colors.RESET
                    + Colors.BRIGHT_CYAN + " is the last process, running to completion" + Colors.RESET + " ["
                    + remainingTime + "ms]");
            ...
            // Task 3: Release semaphore in finally block
            SharedResources.cpuSemaphore.release();
        }
    }
...
public class SchedulerSimulationSync {
    public static void main(String[] args) {
        ...
        int studentID = 445052037; // CHANGE THIS TO YOUR ACTUAL STUDENT ID
        ...
        while (!processQueue.isEmpty()) {
            ...
            Queue<Thread> tempQueue = new LinkedList<>(processQueue);
            boolean first = true;
            while (!tempQueue.isEmpty()) {
                Thread t = tempQueue.poll();
                Process p = processMap.get(t);
                if (!first) {
                    System.out.print(Colors.MAGENTA + " → " + Colors.RESET);
                }
                System.out.print(Colors.CYAN + p.getName() + Colors.RESET);
                first = false;
            }
            ...
            // Start the thread and wait for it to complete its quantum
            currentThread.start();
            try {
                currentThread.join();
            } catch (InterruptedException e) {
                System.out.println(Colors.RED + "Scheduler interrupted!" + Colors.RESET);
            }
            ...
            if (!process.isFinished()) {
                if (!processQueue.isEmpty()) {
                    ...
                    System.out.println(Colors.BRIGHT_YELLOW + " ⚠️" + Colors.CYAN + process.getName() +
